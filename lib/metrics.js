...
var DEFAULT_HIST_AGGREGATES = ['max', 'min', 'sum', 'avg', 'count'];
var DEFAULT_HIST_PERCENTILES = [0.75, 0.85, 0.95, 0.99];
...
function Histogram(key, tags, host, options) {
    ...
    this.count = 0;
    this.samples = [];
    this.aggregates = options ? (options.histAggregates || DEFAULT_HIST_AGGREGATES)
                            : DEFAULT_HIST_AGGREGATES;
    this.percentiles = options ? (options.histPercentiles || DEFAULT_HIST_PERCENTILES)
...
Histogram.prototype.flush = function() {
    ...
    if (this.aggregates.indexOf('count') !== -1) {
        points.push(this.serializeMetric(this.count, 'counter', this.key + '.count'));
    }
    if (this.aggregates.indexOf('avg') !== -1) {
        points.push(this.serializeMetric(this.average(), 'gauge', this.key + '.avg'));
    }
